...
```{r, eval=F}
download.file(url="https://public-docs.crg.es/biocore/projects/training/R_introduction_2021/inputA.txt", destfile = "inputA.txt")
```

2. Read in the content of inputA.txt using the scan command; save in object z
How many elements are in z?

3- Sort z: save sorted vector in a new object "zsorted" (see function **sort()**)

4. Write zsorted content into file outputA.txt, with the **write()** function.
...
* NOTE: **read.csv** derives from the read.table function, and reads **.csv** (comma-separated) files...

**HANDS-ON**

1. Download file **inputB.txt** the following way, using the **download.file** function:

```{r, eval=F}
download.file(url="https://public-docs.crg.es/biocore/projects/training/R_introduction_2021/inputB.txt", destfile = "inputB.txt")
```

2. Read in the content of inputB.txt using the **read.table** command; save in object "pop".
What are the dimensions of "pop"? What are the column names?
